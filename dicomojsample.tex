...
% 所属ラベルの定義
\affiliate{KYOTO}{京都大学 大学院 情報学研究科\\Graduate School of Informatics, Kyoto University}
\affiliate{OIT}{大阪工業大学 大学院 情報科学研究科\\Graduate School of Information Science, Osaka Institute of Technology}


\author{島野 凌}{RYO SHIMANO}{KYOTO}
...
\begin{abstract}
  ...
  これは，IID 環境において，二重異種性を同時に扱うことが精度上のトレードオフを伴わず，
  むしろ既存手法を上回る精度を達成できることを示すものである．
\end{abstract}
...
CNN 端末 6 台のみを対象とした HeteroFL（75.77\%）を 13.12 ポイント上回った．
ただし，両者は参加端末数・アーキテクチャ構成が異なる条件であり，直接比較には留意が必要である．
...
FedGen（全端末 $r=1.0$ のフルサイズモデルを使用）における同縮小率グループの精度よりも AFAD が優位となる傾向が確認された．
...
計算異種性のみに対応する HeteroFL（75.77\%，CNN 6 台）を 13 ポイント以上上回る精度を示した
（ただし参加端末数・構成が異なる条件下での比較）．
この結果は，IID 環境において，二重異種性への同時対応が精度上のトレードオフを伴わないことを示す．
