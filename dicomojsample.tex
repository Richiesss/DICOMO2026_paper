...
\begin{figure*}[htb]
  ...
  \begin{tikzpicture}[
    ...
    gen/.style ={rectangle, draw, fill=purple!15, minimum height=1.4cm, minimum width=2.5cm, inner sep=5pt, align=center},
    agg/.style ={rectangle, draw, fill=gray!25,   minimum height=0.9cm, minimum width=2.5cm, inner sep=5pt, align=center},
  ...
  \node[agg] (agg) at (7.0, 7.2) {HeteroFL\\Aggregation};

  %% Global Model（行全体を x=7 中心に配置）
  \node[bb, minimum width=1.4cm] (gbb)  at (5.85, 5.5) {BB\\{\scriptsize $r\!=\!1.0$}};
  ...
  %% Generator
  \node[gen] (gen) at (7.0, 4.0) {Generator $G$\\{\footnotesize $(\mathbf{z},y)\!\to\!\hat{\mathbf{h}}$}};
  ...
  \end{scope}
  \node[lbl, font=\small\bfseries, above=0.12cm of serverbox.north] {\textbf{Server}};

  %% ===== CLIENTS (bottom, left→right) =====

  %% Client A: CNN r=1.0（左）
  \node[bb]   (bbA) at (0, 0)   {Backbone\\{\scriptsize CNN, $r\!=\!1.0$}};
  ...
  \end{scope}
  \node[lbl, font=\small\bfseries, below=0.12cm of boxA.south] {\textbf{Client A}};

  %% Client B: CNN r=0.25（中央）
  \node[bbSm] (bbB) at (6.0, 0)  {Backbone\\{\scriptsize CNN, $r\!=\!0.25$}};
  ...
  \end{scope}
  \node[lbl, font=\small\bfseries, below=0.12cm of boxB.south] {\textbf{Client B}};

  %% Client C: ViT r=1.0（右）
  \node[bb]   (bbC) at (11.0, 0) {Backbone\\{\scriptsize ViT, $r\!=\!1.0$}};
  ...
  \end{scope}
  \node[lbl, font=\small\bfseries, below=0.12cm of boxC.south] {\textbf{Client C}};

  %% ===== ARROWS (vertical routing) =====
  %% 各矢印は client.north から上へ，または server 要素から下へ出発し，
  %% xshift で4レーンに分けて routing する：
  %%   xshift -0.22cm : backbone upload  (青)
  %%   xshift -0.07cm : Btl+CLS upload   (橙)
  %%   xshift +0.07cm : global model DL  (灰)
  %%   xshift +0.22cm : virtual KD DL    (赤)
  %% 中間の水平パスの高さ（上から）: 2.95, 2.35, 1.26, 0.86 cm

  %% (1) backbone upload → Aggregation [青破線・太]
  \draw[->, blue!80!black, dashed, thick]
    ([xshift=-0.22cm]boxA.north) -- ++(0,0.30) -| ([xshift=-0.22cm]agg.south)
    node[near start, left, lbl] {backbone};
  \draw[->, blue!80!black, dashed, thick]
    ([xshift=-0.22cm]boxB.north) -- ++(0,0.30) -| ([xshift=-0.22cm]agg.south);
  \draw[->, blue!80!black, dashed, thick]
    ([xshift=-0.22cm]boxC.north) -- ++(0,0.30) -| ([xshift=-0.22cm]agg.south);

  %% (2) Btl+CLS upload → Global Model [橙破線]
  \draw[->, orange!80!black, dashed]
    ([xshift=-0.07cm]boxA.north) -- ++(0,0.70) -| ([xshift=-0.07cm]gbtl.south)
    node[near start, right, lbl] {Btl+CLS};
  \draw[->, orange!80!black, dashed]
    ([xshift=-0.07cm]boxB.north) -- ++(0,0.70) -| ([xshift=-0.07cm]gbtl.south);
  \draw[->, orange!80!black, dashed]
    ([xshift=-0.07cm]boxC.north) -- ++(0,0.70) -| ([xshift=-0.07cm]gbtl.south);

  %% (3) global model download → clients [灰実線・太]
  \draw[->, gray!70!black, thick]
    ([xshift=+0.07cm]gcls.south) -- ++(0,-2.80) -| ([xshift=+0.07cm]boxA.north)
    node[near start, right, lbl] {global model};
  \draw[->, gray!70!black, thick]
    ([xshift=+0.07cm]gcls.south) -- ++(0,-2.80) -| ([xshift=+0.07cm]boxB.north);
  \draw[->, gray!70!black, thick]
    ([xshift=+0.07cm]gcls.south) -- ++(0,-2.80) -| ([xshift=+0.07cm]boxC.north);

  %% (4) virtual KD → clients [赤破線・太]
  \draw[->, red!70!black, dashed, thick]
    ([xshift=+0.22cm]gen.south) -- ++(0,-0.40) -| ([xshift=+0.22cm]boxA.north)
    node[near start, left, lbl] {virtual $\hat{\mathbf{h}}$};
  \draw[->, red!70!black, dashed, thick]
    ([xshift=+0.22cm]gen.south) -- ++(0,-0.40) -| ([xshift=+0.22cm]boxB.north);
  \draw[->, red!70!black, dashed, thick]
    ([xshift=+0.22cm]gen.south) -- ++(0,-0.40) -| ([xshift=+0.22cm]boxC.north);

  \end{tikzpicture}
  \caption{AFAD のシステム概要（上：サーバ，下：クライアント群）．
    各端末は幅可変の backbone を持ち，共通の 32次元 bottleneck 層（Btl 32）を通じてサーバと知識を共有する．
