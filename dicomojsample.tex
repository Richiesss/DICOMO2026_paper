...
\begin{abstract}
  ...
  OrganAMNIST を用いた IID 環境での評価により，AFAD+Proto はサーバ精度 82.93\% を達成した．
  同一の二重異種性条件下での HeteroFL（82.20\%）を上回り，
  二重異種性への同時対応が HeteroFL 比での精度低下を伴わないことを示した．
  FedGen（86.32\%）との差 3.4 ポイントは今後の課題として残るが，
  HeteroFL が参加を許容しない ViT 端末を含む全 10 台が参加する，
  より困難な設定でこの結果を達成したことを強調する．
\end{abstract}
...
\begin{itemize}
  ...
  \item OrganAMNIST を用いた IID 環境での実験により，
        AFAD+Proto がサーバ精度 82.93\% を達成し，HeteroFL（82.20\%）を上回ることを示す．
...
\begin{table}[htb]
  \centering
  \caption{OrganAMNIST IID 環境における精度比較（サーバ精度，Best, rate=1.0 グローバルモデル）}
  \label{tab:results}
  \begin{tabular}{lp{2.0cm}cc}
    ...
    \textbf{AFAD+Proto（提案）}   & \textbf{全端末（10台）} & \textbf{あり} & \textbf{82.93} \\
    \bottomrule
  \end{tabular}
\end{table}

AFAD+Proto はサーバ精度 82.93\% を達成し，
同一の二重異種性条件下での HeteroFL（82.20\%）を上回った．
AFAD は ViT 端末を含む全 10 台が参加する，
より困難な二重異種性環境での評価であるにもかかわらず，
HeteroFL と同等以上の精度を達成したことは提案手法の有効性を示す．
...
AFAD+Proto はサーバ精度 Mean R11+ = 82.60\% を達成し，
AFAD Hybrid（81.66\%）および HeteroFL（81.91\%）を上回った．
AFAD+Proto は最終ラウンドまで安定した精度を維持しており，
再現性の高い改善が確認された．
...
\begin{figure}[htb]
  ...
  \caption{各手法のサーバ精度の収束曲線（40 ラウンド）．
    AFAD+Proto は HeteroFL を上回り安定収束する．}
