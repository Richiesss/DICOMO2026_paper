...
\begin{abstract}
  ...
  OrganAMNIST を用いた IID 環境での評価により，AFAD は 88.89\% の精度を達成した．
  同一の 10 端末・同一データ量という公平な条件で比較した FedGen（86.32\%）を 2.57 ポイント上回り，
  二重異種性への同時対応が精度上のトレードオフを伴わないことを示した．
  また参考として，CNN 端末 6 台のみを対象とした HeteroFL（75.77\%）と比較すると
  13.12 ポイント上回るが，この比較は参加端末数・アーキテクチャ構成が異なる条件である．
\end{abstract}
...
図~\ref{fig:per_rate}に，各手法の IID 環境における Mean Accuracy（ラウンド 11〜40 の平均，クライアント評価）を示す．

\begin{figure}[htb]
  \centering
  \includegraphics[width=0.95\linewidth]{figs/per_rate.pdf}
  \caption{各手法の Mean Accuracy（R11--40 平均，クライアント評価）の比較．
    AFAD+Proto は AFAD Hybrid および HeteroFL を上回り，
    安定した精度向上を示した．}
  \label{fig:per_rate}
\end{figure}

AFAD+Proto は AFAD Hybrid（75.75\%）および HeteroFL（75.81\%）を上回る 77.47\% を達成した．
AFAD+ProjHead は最高精度 88.89\% を記録するものの，収束が不安定であり
Mean R11+ は 72.52\% にとどまる．
一方 AFAD+Proto は最終ラウンドまで安定した精度を維持しており，
再現性の高い改善が確認された．
...
\begin{figure}[htb]
  ...
  \caption{各手法のクライアント精度の収束曲線（40 ラウンド）．
    AFAD+Proto は HeteroFL より高い精度に安定収束する．}
  \label{fig:convergence}
\end{figure}

AFAD+Proto は初期ラウンドから安定した精度向上を示し，
ラウンド 11 以降は 77\% 前後で安定収束した．
HeteroFL（75.81\%）を上回る一方，
FedGen（85.74\%）との差は依然として存在し，
...
AFAD は 88.89\% の精度を達成した．
同一の 10 端末・同一データ量という公平な条件で比較した
FedGen（86.32\%）を 2.57 ポイント上回り，
二重異種性の同時対応が精度上のトレードオフを伴わないことを実証した．
また，CNN 端末 6 台のみを対象とした HeteroFL（75.77\%）も大幅に上回ったが，
参加端末数・構成が異なる条件での比較である点に留意が必要である．
