...
\begin{figure}[t]
  ...
  \node[bn, right=0.15cm of bbA] (bnA) {BN\\32};
  \node[cls, right=0.15cm of bnA] (clsA) {CLS};

  %% ---- Client B: CNN r=0.25 ----
  \node[bbnarrow, minimum width=0.65cm] (bbB) at (0, 1.2) {BB\\$r\!=\!.25$};
  \node[bn, right=0.15cm of bbB] (bnB) {BN\\32};
  \node[cls, right=0.15cm of bnB] (clsB) {CLS};

  %% ---- Client C: ViT r=1.0 ----
  \node[bb, minimum width=1.3cm] (bbC) at (0, 0) {Backbone\\(ViT, $r\!=\!1.0$)};
  \node[bn, right=0.15cm of bbC] (bnC) {BN\\32};
  ...
  \node[bn, right=0.12cm of gbb] (gbn) {BN\\32};
