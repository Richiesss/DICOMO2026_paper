...
\subsection{プロトタイプベース連合学習}

各クラスの特徴空間における代表ベクトル（プロトタイプ）を用いた知識共有も
有効なアプローチとして注目されている．
Tan ら\cite{FedProto}は，各端末がローカルデータからクラスプロトタイプを計算し，
サーバが集約したグローバルプロトタイプを端末間の知識共有媒体とする
FedProto を提案した．
FedProto はアーキテクチャに依存しない潜在空間での知識共有を実現するが，
計算異種性（モデル幅の差異）への対応は考慮されていない．
AFAD はこのプロトタイプ正則化の考え方を採り入れ，
共有ボトルネック空間における CNN と ViT の潜在表現の整合性向上に活用している．
...
AFAD はさらに，FedProto\cite{FedProto}の考え方を参考に，
各クラスのボトルネック特徴の重心（プロトタイプ）を用いた
