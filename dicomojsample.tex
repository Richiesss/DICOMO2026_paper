...
\begin{abstract}
  ...
  FedGen（86.32\%）を上回るとともに，CNN 端末のみを対象とした HeteroFL（75.77\%）を
  大幅に上回る精度を達成した．ただしこの比較は参加端末数・アーキテクチャ構成が異なる条件である．
...
\begin{table}[t]
  ...
  \caption{連合学習関連手法の比較}
  \label{tab:related}
  \begin{tabular}{lp{1.2cm}p{1.8cm}p{1.5cm}}
    \toprule
...
$\hat{\mathbf{h}} = G(\mathbf{z}, y)$ を仮想潜在ベクトルとすると，
...
\begin{table}[t]
  \centering
  \caption{OrganAMNIST IID 環境における精度比較（3シード平均）}
  \label{tab:results}
