...
\begin{itemize}
  ...
  \item OrganAMNIST を用いた IID 環境での実験により，
        AFAD が FedGen（86.32\%）を上回り，HeteroFL（75.77\%）を大幅に超える
        精度（88.89\%）を達成することを示す．
\end{itemize}
...
なお，本データセットは MedMNIST v1 における「OrganMNIST\_Axial」に相当し，
v2 で OrganAMNIST に改称された．

OrganAMNIST は医療画像分野の複雑なデータでありながら，
MNIST と同等の軽量・標準化された形式をとるため，
データパイプラインの構築コストを最小化しつつアルゴリズムの評価に集中できる点が特徴である．
また，複数拠点（病院等）にデータが分散している状況をシミュレーションしやすく，
連合学習のベンチマークとして広く利用されている．
